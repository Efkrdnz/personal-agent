...
This package is deliberately EMPTY of imports. ``jarvis.cc.narrate``,
``jarvis.cc.policy``, ``jarvis.cc.settings`` and ``jarvis.cc.gate`` are pure
standard-library-plus-spine and must stay importable on a machine that has never
installed ``claude-agent-sdk``; only :mod:`jarvis.cc.permission_host` and
